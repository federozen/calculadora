...
| `lpf_data_provider.py` | Contrato `DataProvider` v2: cualquier fuente entrega `ProviderData` canónico y JSON-safe. Incluye `CurrentProvider` y `CsvProvider`; un futuro Opta sólo implementa la misma interfaz. |
...
| `lpf_editorial_definition.py` | Composición pura de definición: contexto Playoffs/Copas, zona de pelea, matrices, garantía, reloj y paquete JSON-safe reutilizable por Streamlit o servicios. |
...
13. **Composición de definición (`lpf_editorial_definition`):** convierte esos condicionales y el contexto de objetivo en zona de pelea, matrices, garantía/reloj y un paquete serializable sin leer Streamlit.

14. **Descenso actual (`lpf_relegation`):** separa descendidos confirmados de desempates y resuelve la duplicación promedio→Anual.
15. **Redacción y UI:** los renderizadores consumen los resultados estructurados.

## Frontera para futura API y proveedores externos

La aplicación debe conservar una frontera simple y estable:

`fuente → transporte/adaptador → DataProvider → ProviderData → snapshot/estado LPF canónico → lpf_services/motores → API o Streamlit`
...
- `lpf_data_provider` fija la entrada común de fuente: `DataProvider.load() -> ProviderData`. Streamlit usa `CurrentProvider`; `CsvProvider` es la implementación reproducible de referencia. Ningún proveedor contiene reglas de Playoffs, Copas o descenso.
...
- Una futura API debe entrar por la superficie pública de `lpf_services.calculate()`;
  no debe importar `calculadora_futbol_argentino.py` ni reconstruir bases/cortes que ya
  están disponibles en el snapshot. Los motores (`lpf_standings`, `lpf_scenarios`,
  `lpf_exact`, `lpf_pisos`, etc.) quedan detrás de esa frontera.
...
La interfaz genérica de proveedores ya existe como `DataProvider` v2. No se agrega
todavía FastAPI ni un SDK de Opta: cuando exista ese consumidor, sólo deberá producir
`ProviderData` y reutilizar el snapshot y `lpf_services` existentes.
...
delega en los motores puros. Desde 3.8.43 también expone `calculate_definition`, que consume el paquete de `lpf_editorial_definition` para que una futura API pueda reproducir el tablero exacto sin importar Streamlit. Desde 3.8.44, `lpf_snapshot.py` publica schema 2: además de la foto canónica de competencia conserva el contexto de Playoffs/Libertadores/Sudamericana (elegibles, vías directas y corte efectivo). Desde 3.8.45, `calculate_definition` y `competition_batch(type=definition)` pueden resolver además la fecha desde el fixture/pending del snapshot, de modo que el cliente sólo aporte equipo + objetivo (+ zona para Playoffs) + fecha opcional. Desde 3.8.47 queda congelada una **superficie pública mínima v1** mediante `lpf_services.calculate()` con siete operaciones: `standings`, `preview`, `objective_points`, `objective_chances`, `definition`, `relegation` y `competition_batch`. Los helpers legacy siguen disponibles pero ya no son el contrato recomendado para una integración nueva. Una futura API HTTP debe llamar esta frontera en vez de importar `calculadora_futbol_argentino.py`. Desde 3.8.48, las rutas principales de Streamlit también llaman esa frontera pública; los fallbacks legacy quedan registrados en auditoría y las excepciones de rendimiento se documentan explícitamente. Desde 3.8.50, el snapshot canónico sube a schema 3 e incorpora trazabilidad de fuente, cobertura, calidad y un `snapshot_id` estable; `DataProvider` v2 agrega `provenance` sin permitir que el proveedor contenga reglas de competencia.
