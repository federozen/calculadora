...
def test_copas_por_puntos_dejan_claro_que_la_via_es_tabla_anual():
    from lpf_pisos import objetivos_de_equipo

    zonas = {"A": {"A": {"pts": 10}, "B": {"pts": 9}}}
    anual = {"A": {"pts": 20}, "B": {"pts": 19}, "C": {"pts": 18}}
    specs = objetivos_de_equipo(zonas, anual, ["A", "B", "C"], 1, "A")
    nombres = {spec["clave"]: spec["nombre"] for spec in specs}
    assert nombres["libertadores"] == "la Libertadores por Tabla Anual"
    assert nombres["sudamericana"] == "al menos la Sudamericana por Tabla Anual"


def test_equipo_con_plaza_directa_no_desaparece_de_puntos_por_objetivo():
    from lpf_pisos import pisos_de_equipo
    ...
    rest = {"Campeon": 2, "Rival": 2, "Otro": 2}
    # ``Campeon`` no está en la reducida porque ya obtuvo una plaza directa.
    pisos = pisos_de_equipo(
        zonas, anual, ["Rival", "Otro"], 1, rest, [], "Campeon", n_anual=1,
    )
    por_clave = {p.clave: p for p in pisos}

    assert {"libertadores", "sudamericana"} <= set(por_clave)
    lib = por_clave["libertadores"]
    sud = por_clave["sudamericana"]
    assert lib.estado == "in"
    assert sud.estado == "in"
    assert lib.piso is None
    assert sud.piso is None
    assert "vía directa" in lib.lectura()
    assert "Libertadores" in sud.lectura()
    assert "con 30 puntos" not in lib.lectura()


def test_lectura_de_objetivo_cumplido_puede_explicar_una_via_no_basada_en_puntos():
    p = PisoObjetivo(
        clave="libertadores",
        nombre="la Libertadores",
        estado="in",
        puntos_hoy=30,
        techo=36,
