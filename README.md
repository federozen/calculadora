...
La suite actual contiene **290 pruebas**.

| Archivo | Qué cubre |
| --- | --- |
| `test_lpf_conditionals.py` | Próxima fecha exacta: filtro de partidos relevantes/interzonales, ramas G/E/P, frecuencia combinatoria y condiciones simples suficientes. |
| `test_lpf_relegation.py` | Descenso actual: duplicación promedio→Anual, empates reglamentarios sin DG y relato editorial coherente. |
| `test_lpf_definition_radar.py` | UI del tablero final: ubicación, ventana de activación, rótulo no probabilístico y separación del Monte Carlo ESTIMADO. |
...
| `test_lpf_data_provider.py` | Contrato DataProvider común: fuente actual y CSV producen el mismo payload/snapshot canónico; validación de fixture, interzonales y errores de columnas. |
...
| `test_lpf_pisos.py` | Puntos por objetivo: mínimo posible y garantía por objetivo, invariantes, combinación de tablas en el descenso y separación entre vía Anual y plazas directas de copas. |
...
En 3.8.37 se agregaron regresiones para que los pisos de copas se rotulen como vía de **Tabla Anual**, los clasificados directos no desaparezcan y la foto real de Belgrano (campeón del Apertura) siga mostrándolo como clasificado sin inventar un mínimo de puntos.

En 3.8.38 se agregaron regresiones para la matriz exacta **Qué tiene que pasar** y para que ningún relato de descenso resuelva por diferencia de gol una igualdad que reglamentariamente exige partido desempate.
