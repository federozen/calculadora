...
- La actualización automática suma un proveedor de resultados de TyC Sports antes de ESPN/FutbolArgentino.com. La página de fixture del Clausura concentra los resultados fecha por fecha y se usa únicamente para marcadores; las tablas y reglas no se toman de allí.
- El parser entra sólo en la sección `Resultados del Torneo Clausura 2026`, exige `Fecha N`, marcador explícito y pareja/round válidos en `LPF_FIXTURE`. El fixture futuro y los módulos relacionados no pueden convertirse en partidos jugados.
- La prioridad queda: manual > LPF oficial > TyC > base anterior > base incluida > FutbolArgentino.com > ESPN. Si LPF + TyC ya explican exactamente PJ/puntos/GF/GC/DG, no se consultan ESPN/FutbolArgentino.
- Se corrige además el hostname de LPF de `www.lpf.org.ar` a `lpf.org.ar`; no se desactiva la verificación TLS.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 siguen estables.

## Novedad 3.8.69 · tabla vigente como estado autoritativo

- Cambia la estrategia para el caso real `tabla = 135 / resultados confirmados = 49`: la calculadora ya no exige reconstruir los 135 marcadores históricos para operar cuando la tabla publicada y el fixture permiten separar con seguridad pasado y futuro.
- Si todos los clubes tienen el mismo PJ, el fixture hasta ese frente contiene exactamente ese número de partidos por equipo, no hay resultados explícitos de fechas posteriores y los marcadores parciales son compatibles con PJ/puntos/GF/GC, los partidos históricos sin score quedan `unconfirmed` como advertencia y no bloquean.
- Con una foto uniforme de 9 PJ, Fechas 1-9 quedan históricas y Fechas 10-16 quedan pendientes. No se inventa ningún marcador.
- Los resultados parciales se conservan para forma, racha y auditoría; la tabla vigente gobierna el estado competitivo actual.
- La fuente oficial migra a `lpf.org.ar` y `/categoria/primera/`, pero pasa a funcionar como backfill/auditor en lugar de dependencia dura.
- Si los PJ son desparejos, hay adelantados/postergados detectables o los resultados parciales contradicen la tabla, el sistema sigue fallando cerrado.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.

## Novedad 3.8.68 · LPF oficial: soporte para fechas publicadas con `<br>`

- Corrige el caso real `tabla = 135 / LPF oficial = 0`: la web oficial publica algunas notas con toda la fecha dentro de un mismo bloque HTML y separa cada línea con `<br>`.
- El aislamiento de 3.8.67 se mantiene: el parser sigue limitado a `article/main`, conserva la guarda Apertura/Clausura y exige que cada marcador pertenezca al `expected_round`.
- Dentro de ese cuerpo se vuelven a leer fragmentos atómicos (`stripped_strings`), por lo que un párrafo con 15 partidos separados por `<br>` vuelve a producir 15 resultados en lugar de una cadena gigante imposible de parsear.
- Se agrega respaldo para líneas divididas por tags inline (`strong/span`) sin aceptar contenedores con múltiples marcadores.
- Regresión nueva: una Fecha 9 completa en un único `<p>` con `<br>` debe devolver exactamente 15 partidos y no absorber un marcador del `aside`.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.

## Novedad 3.8.67 · LPF oficial: cada nota queda atada a su fecha

- Corrige el caso `tabla = 135 / LPF oficial = 92`: Fechas 4-9 son 90 partidos, por lo que esos dos extras demostraban contaminación dentro del HTML de las propias páginas.
- El parser ya no recorre toda la página WordPress: sólo procesa el cuerpo principal del artículo y no puede leer marcadores de módulos laterales/relacionados.
- Cada semilla oficial de Fecha 4-9 lleva su `expected_round`. Un marcador sólo entra si la pareja existe en `LPF_FIXTURE` **y pertenece a esa misma fecha**.
- El crawler futuro infiere la fecha desde el título/URL cuando es inequívoca y aplica la misma guarda. Sin contexto de fecha, un marcador automático no se publica.
- Regresión de aceptación: seis notas × 15 partidos + cards contaminantes deben devolver exactamente **90 resultados**; unidos a la base validada de 49 con cuatro solapamientos de Fecha 4, explican 135 partidos.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.



## Novedad 3.8.66 · LPF oficial sin contaminación del Apertura

- Corrige el caso real `tabla = 135 / LPF oficial = 100`: al ampliar el archivo en 3.8.65 podían entrar notas del **Apertura 2026**. Como varias parejas de clubes se repiten en el Clausura, un marcador viejo podía pasar la guarda de fixture y adelantar PJ de equipos como Rosario Central o Estudiantes (RC).
- Los permalinks fechados de Primera sólo se aceptan desde el inicio del Clausura 2026. Un artículo que se identifica como `Torneo Apertura` se descarta completo aunque la pareja también exista en `LPF_FIXTURE`.
- Los permalinks cortos `/?p=...` dejan de ser abiertos: sólo se aceptan los IDs auditados de Fechas 4, 5 y 6.
- La foto actual prioriza seis notas oficiales auditadas de **Fechas 4 a 9**. Con la base incluida de 49 partidos, esa unión explica exactamente los 135 partidos de nueve fechas y evita bajar al archivo histórico cuando no hace falta.
- El archivo paginado queda como mecanismo de descubrimiento para las fechas siguientes, con corte temprano cuando la unión ya explica los PJ publicados.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 siguen estables.
...
- Los marcadores partido a partido se buscan primero en **Liga Profesional de Fútbol (sitio oficial)**. ESPN y FutbolArgentino.com pasan a fallback cuando la fuente oficial no alcanza.
- Las notas oficiales sólo aportan un resultado si contienen marcador explícito y la pareja existe en `LPF_FIXTURE`; programación, titulares y prosa no se convierten en partidos jugados.
- Si LPF oficial + base validada reconstruyen la tabla, la carga evita consultas innecesarias a ESPN/FutbolArgentino.com. Esto reduce los bloqueos por HTTP 403 y cambios de HTML de terceros.
- Caso cubierto: `49 resultados incluidos + 42 marcadores oficiales de Fechas 4-6 = tabla de 87 partidos`, sin inferir resultados.
- Como respaldo excepcional, los saltos de 3-4 PJ por club pueden conciliarse con **MILP exacto**; una segunda resolución debe demostrar que no existe otra combinación antes de publicar marcadores inferidos.
- La conciliación sigue siendo auditor/respaldo. En producción con Opta, el proveedor explícito de identidad/status/marcador continúa siendo la fuente primaria.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.

## Novedad 3.8.62 · Conciliación sin tope fijo 16 y diagnóstico de fuentes

- Corrige el nuevo caso real `49 resultados validados → tabla que implica 67`: la conciliación determinística puede intentar los **18 partidos faltantes** si fixture + PJ/puntos/GF/GC/DG fijan una única solución.
- Se elimina el tope fijo de 16 partidos. La seguridad pasa a depender de la **complejidad real de la búsqueda**: máximo 2 PJ nuevos por club, ventana de fechas consecutivas y un presupuesto determinístico de estados. Si la búsqueda se vuelve demasiado abierta, se abandona sin inferir.
- Una segunda solución compatible sigue invalidando toda la reconstrucción. La app informa explícitamente si el bloqueo fue por ambigüedad o por presupuesto de búsqueda.
- El caso 49→67 queda cubierto por regresión y reconstruye 18 resultados en dos fechas parciales sin convertir la tabla en una fuente especulativa.
- El error de actualización ahora expone mejor el problema de cada feed: si ESPN/FutbolArgentino devuelve filas que luego no coinciden con clubes/fixture, se informa el conteo descartado; los errores de fuente y de conciliación dejan de quedar escondidos detrás de un simple `aportó 0`.
- Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.
...
- Si una fecha tiene más de 8 otras canchas relevantes, G/E/P deja de depender de enumerar `3^N`: usa el solver exacto MILP sobre todo el fixture pendiente.
- Verde sigue significando garantía matemática; rojo, eliminación matemática; amarillo, objetivo abierto. No se usa Monte Carlo para esas celdas.
- El límite del enumerador detallado ya no oculta el **termómetro**, el **heatmap de Copas** ni el **impacto estimado de otras canchas**.
- La doble entrada y el árbol exacto se omiten sólo cuando no pueden enumerarse de forma razonable, con explicación explícita.
- La misma regla se aplica en `Últimas fechas` y `Visualizaciones → Copas y descenso`.

## Novedad 3.8.59 · Copas recupera G/E/P y el termómetro individual

- `Visualizaciones → Copas y descenso` suma la misma **matriz exacta G/E/P** que Playoffs, aplicada al corte de la Tabla Anual.
- Si el objetivo está abierto, G/E/P reutiliza `definition`; si ya está resuelto por vía directa, las tres ramas se muestran verdes y explican que el resultado de la fecha no modifica el cupo.
- El **termómetro “¿Cómo viene?”** vuelve a mostrarse siempre: ESTIMADO · 6.000 simulaciones cuando el objetivo está abierto; **OBJETIVO RESUELTO** cuando la clasificación es directa y exacta.
- `Últimas fechas` también conserva esos dos visuales para un clasificado directo y permite abrir el heatmap comparativo de Copas sin convertir el 100% exacto en una simulación.
- El heatmap rojo→amarillo→verde y la otra cancha de 3.8.58 permanecen intactos. Public Service v1, DataProvider v2, Snapshot schema 3 y Runtime API 21 no cambian.
...
- Corrige el crash de Streamlit al usar **Agregar sugeridos / Quitar comparadores**: los cambios del multiselect ahora pasan por callbacks y no mutan el estado del widget después de instanciarlo.
- `Últimas fechas` expone de forma visible más ideas de la calculadora del Mundial: **mapa exacto de puestos tras la fecha**, **cara a cara**, barra de **partidos que más definen** y **termómetro estimado de chances**.
- La grilla G/E/P, la doble entrada y el árbol siguen siendo exactos; el visual de chances queda separado y rotulado como **ESTIMADO · 6.000 simulaciones**.
- No cambia Public Service v1, DataProvider v2, Snapshot schema 3 ni Runtime API 21.


## Novedad 3.8.54 · Developer / Production Bootstrap

- Nueva capa `api/` con FastAPI **fina**: health, readiness, capabilities y endpoints HTTP para las siete operaciones de Public Service v1. Toda la matemática sigue en `lpf_services.calculate()`.
- La API acepta snapshot en el request o puede inyectarlo desde `LPF_SNAPSHOT_FILE`, de modo que staging puede levantarse antes de tener Opta.
- Nuevo `providers/opta.py`: contrato `OptaTransport + OptaNormalizer -> OptaProvider -> ProviderData`, sin inventar el schema del producto Opta contratado.
- `pyproject.toml` declara explícitamente el runtime empaquetable; `api`/`providers` entran al wheel y tests/legacy/data/históricos quedan fuera. Streamlit pasa al extra `ui`; la API puede instalar `.[api]` sin depender de Streamlit.
- `Dockerfile.api`, `.env.example` y `Makefile` dejan comandos de arranque/check estables.
- `tools/build_runtime_bundle.py` genera un ZIP mínimo de runtime/API sin tests, legacy ni históricos.
- Nuevo acceptance kit: siete casos dorados, smoke HTTP y `make acceptance`; `PRODUCTION_BOOTSTRAP.md` incluye Definition of Done y cronograma orientativo de staging/producción.
- Public Service v1, DataProvider v2, snapshot schema 3 y Runtime API 21 **no cambian**. Validación: **361 pruebas**, acceptance kit verde y build de wheel OK.
